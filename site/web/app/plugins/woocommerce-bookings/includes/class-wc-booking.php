...
class WC_Booking {
		...
		if ( $this->populated ) {
			if ( in_array( $status, $allowed_status_keys ) ) {
				...
				if ( $order = $this->get_order() ) {
					$order->add_order_note( sprintf( __( 'Booking #%d status changed from "%s" to "%s"', 'woocommerce-bookings' ), $this->id, $allowed_statuses[ $current_status ], $allowed_statuses[ $status ] ) );
				}
	...
	/**
	 * Returns the object of the order corresponding to this booking
	 * @return WC_Order | Boolean . False if booking is not populated
	 */
